...
public class Util {
	public static InetAddress getLoopbackAddress() {
		try {
			...
			Main.println("[Util] Failed to get localhost address");
			return null;
		}
	}
	...
	public static byte[] readFile(File f) {
		try {
			...
			while(i < data.length) {
				...
				if(len == -1) {
					Main.println("[Util] Warning: may not have read fully on " + f.getAbsolutePath());
					break;
				}
			}
			...
			Main.println("[Util] Error while reading " + f.getAbsolutePath() + ": " + ioe.getLocalizedMessage());
			return new byte[] {};
		}
	}
	...
	public static MessageDigest getSha1Digest() {
        try {
            ...
            Main.println("[Util] Fatal error: SHA-1 not available");
			return null;
        }
	}
	...
	public static String readTerminatedString(InputStream in) {
		try {
			...
			Main.println("[Util] Error: unable to read from input stream");
			return null;
		}
	}
