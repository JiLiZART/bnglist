...
public class BNCSUtilInterface {
	...
	public boolean HELP_SID_AUTH_CHECK (boolean TFT, String war3Path, String keyROC, String keyTFT, String valueStringFormula, String mpqFileName, int clientToken, int serverToken ) {
		...
		if(keyROC.length() != 26 || (TFT && keyTFT.length() != 26)) {
			Main.println("[BNCS] One or both keys are invalid (not 26 characters)");
		}
		...
		if( ExistsWar3EXE && ExistsStormDLL && ExistsGameDLL )
		{
			...
			else
			{
				if( m_KeyInfoROC.length != 36 )
					Main.println("[BNCS] unable to create ROC key info - invalid ROC key");

				if( TFT && m_KeyInfoTFT.length != 36 )
					Main.println("[BNCS] unable to create TFT key info - invalid TFT key" );
			}
		}
		else
		{
			if( !ExistsWar3EXE )
				Main.println("[BNCS] unable to open [" + FileWar3EXE + "]" );

			if( !ExistsStormDLL )
				Main.println("[BNCS] unable to open [" + FileStormDLL + "]" );

			if( !ExistsGameDLL )
				Main.println("[BNCS] unable to open [" + FileGameDLL + "]" );
		}
	...
	private byte[] CreateKeyInfo(String key, int clientToken, int serverToken ) {
		try {
			...
			Main.println("[BNCS] Key hash failed: " + e.getLocalizedMessage());
			return new byte[] {};
		}
	}
}
